...
const TARGETS = [
  { key: 'virus', label: 'Virus / Mothercell', toggleLabel: 'Enable Virus Colors' },
  { key: 'pellets', label: 'Food-Pellets', toggleLabel: 'Enable Pellet colors' },
];
...
export class VirusPelletColorSettingsUi {
  ...
  create() {
    ...
    this.elements = {
      ...
      targetEnabledInputs: Array.from(menu.querySelectorAll('.blobio-virus-pellet-target-enabled-input') || []),
  ...
  createTargetControls(target) {
    ...
    section.appendChild(this.createTargetToggle(target));
  ...
  createTargetToggle(target) {
    const row = this.document.createElement('label');
    row.classList.add('blobio-virus-pellet-target-toggle');
    row.dataset.target = target.key;

    const input = this.document.createElement('input');
    input.type = 'checkbox';
    input.classList.add('blobio-virus-pellet-target-enabled-input');
    input.dataset.target = target.key;

    const text = this.document.createElement('span');
    text.textContent = target.toggleLabel;
    row.append(input, text);

    this.listen(input, 'change', () => {
      this.settings = this.saveTarget(target.key, { enabled: Boolean(input.checked) });
      this.sync();
    });
  ...
  save(changes) {
    ...
    this.refreshRuntime(saved);
    return saved;
  }

  resolvePageWindow() {
    const view = this.document?.defaultView || null;
    if (view?.unsafeWindow && typeof view.unsafeWindow === 'object') {
      return view.unsafeWindow;
    }
    if (typeof unsafeWindow !== 'undefined' && unsafeWindow && typeof unsafeWindow === 'object') {
      return unsafeWindow;
    }
    return view || globalThis;
  }

  refreshRuntime(settings) {
    const detail = { settings, updatedAt: Date.now() };
    const candidates = [
      this.pageWindow,
      this.document?.defaultView,
      this.document?.defaultView?.unsafeWindow,
    ...
    for (const candidate of candidates) {
      ...
      seen.add(candidate);
      if (typeof candidate.__blobioVirusPelletColorRefresh === 'function') {
        candidate.__blobioVirusPelletColorRefresh(settings);
      }
      if (candidate.__blobioVirusPelletColorLoaderStatus) {
        candidate.__blobioVirusPelletColorLoaderStatus.enabled = Boolean(settings.enabled);
        candidate.__blobioVirusPelletColorLoaderStatus.runtimeRefreshAt = detail.updatedAt;
      }
      this.dispatchRuntimeRefreshEvent(candidate, detail);
    }
    this.dispatchRuntimeRefreshEvent(this.document, detail);
  }

  dispatchRuntimeRefreshEvent(target, detail) {
    ...
    try {
      ...
      target.dispatchEvent(new EventCtor(SETTINGS_EVENT, { detail }));
    } catch {
      /* Direct refresh remains the primary path. */
    }
  }
  ...
  sync() {
    ...
    for (const target of TARGETS) {
      ...
      const isEnabled = targetSettings.enabled !== false;

      for (const input of this.elements.targetEnabledInputs.filter((node) => node.dataset.target === target.key)) {
        input.checked = isEnabled;
      }

      for (const section of this.elements.targetSections.filter((node) => node.dataset.target === target.key)) {
        this.setClass(section, 'is-target-disabled', !isEnabled);
      }
  ...
  drawPellets(context) {
    const fill = this.settings.pellets.enabled === false ? 'rgba(255, 180, 80, 0.76)' : this.createCanvasFill(context, this.settings.pellets, {
      x: 0,
      y: 0,
      width: 420,
      height: 300,
  ...
  drawTintedVirus(context, image, virus) {
    ...
    if (this.settings.virus.enabled !== false) {
