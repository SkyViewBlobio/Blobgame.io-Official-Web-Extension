...
const HUD_INFO_BOOSTER_COLOR_MODES = new Set(['solid', 'simple', 'individual']);
const HUD_INFO_BOOSTER_DURATION_COLOR_MODES = new Set(['solid', 'simple']);
...
const HUD_INFO_BOOSTER_TYPES = ['MERGE', 'VIRUS', 'SPEED'];
...
const DEFAULT_HUD_INFO_RUNTIME_SETTINGS = Object.freeze({
  ...
  boosterMergeColor: '#4a7eff',
  boosterMergeAlpha: 1,
  boosterSpeedColor: '#67cfff',
  boosterSpeedAlpha: 1,
  boosterVirusColor: '#ff4a4a',
  boosterVirusAlpha: 1,
...
export function pageHudInfoBootstrap(initialSettings, pageWindow = globalThis) {
  ...
  if (win.__blobioHudInfoInstalled && win.__blobioHudInfoRuntimeVersion === HUD_INFO_RUNTIME_VERSION) {
    win.__blobioHudInfoRefresh?.(initialSettings);
    return true;
  }
  if (win.__blobioHudInfoInstalled) {
    cleanupExistingHudRuntime(win, doc);
  }
  win.__blobioHudInfoInstalled = true;
  win.__blobioHudInfoRuntimeVersion = HUD_INFO_RUNTIME_VERSION;
  ...
  function renderBoosterValue(valueNode, boosters, settings) {
    ...
    active.forEach((booster, index) => {
      ...
      if (settings.boosterNameMode === 'individual') {
        name.classList.add('is-individual-color');
        hudInfoSetStyleColor(name, hudInfoBoosterNameColor(booster.type, settings));
      } else {
  ...
  function debugReport() {
    ...
    return {
      ...
      dom: {
        ...
        boosterValue: boosterValue || null,
      },
      runtimeVersion: HUD_INFO_RUNTIME_VERSION,
      patch: {
...
function cleanupExistingHudRuntime(win, doc) {
  for (const node of doc.querySelectorAll?.('.blobio-hud-info-root, #blobio-hud-info-style') || []) {
    node.remove?.();
  }
  try {
    delete win.__blobioHudInfoInstalled;
    delete win.__blobioHudInfoRuntimeVersion;
    ...
    win.__blobioHudInfoRefresh = undefined;
  }
}
...
function normalizeHudInfoSettings(settings = {}) {
  ...
  return {
    ...
    boosterDurationMode: HUD_INFO_BOOSTER_DURATION_COLOR_MODES.has(source.boosterDurationMode)
    ...
    boosterMergeColor: hudInfoNormalizeColor(source.boosterMergeColor, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterMergeColor),
    boosterMergeAlpha: hudInfoClampNumber(source.boosterMergeAlpha, 0, 1, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterMergeAlpha),
    boosterSpeedColor: hudInfoNormalizeColor(source.boosterSpeedColor, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterSpeedColor),
    boosterSpeedAlpha: hudInfoClampNumber(source.boosterSpeedAlpha, 0, 1, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterSpeedAlpha),
    boosterVirusColor: hudInfoNormalizeColor(source.boosterVirusColor, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterVirusColor),
    boosterVirusAlpha: hudInfoClampNumber(source.boosterVirusAlpha, 0, 1, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterVirusAlpha),
...
function hudInfoRgbaFromSettings(settings) {
  return hudInfoRgbaFromColorAlpha(settings.color, settings.alpha, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.color);
}

function hudInfoRgbaFromColorAlpha(color, alpha, fallbackColor) {
  const clean = hudInfoNormalizeColor(color, fallbackColor).slice(1);
  ...
  return `rgba(${red}, ${green}, ${blue}, ${Math.round((Number(alpha) || 0) * 1000) / 1000})`;
}

function hudInfoBoosterNameColor(type, settings = {}) {
  const normalizedType = String(type || '').toUpperCase();
  const source = normalizeHudInfoSettings(settings);
  if (normalizedType === 'MERGE') {
    return hudInfoRgbaFromColorAlpha(source.boosterMergeColor, source.boosterMergeAlpha, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterMergeColor);
  }
  if (normalizedType === 'VIRUS') {
    return hudInfoRgbaFromColorAlpha(source.boosterVirusColor, source.boosterVirusAlpha, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterVirusColor);
  }
  if (normalizedType === 'SPEED') {
    return hudInfoRgbaFromColorAlpha(source.boosterSpeedColor, source.boosterSpeedAlpha, DEFAULT_HUD_INFO_RUNTIME_SETTINGS.boosterSpeedColor);
  }
  return '';
}
...
function hudInfoOrderBoosters(boosters) {
  const byType = new Map();
  for (const booster of Array.isArray(boosters) ? boosters : []) {
    const type = String(booster?.type || '').toUpperCase();
    if (HUD_INFO_BOOSTER_TYPES.includes(type) && Number(booster.seconds) > 0) {
      byType.set(type, { ...booster, type });
    }
  }
  return HUD_INFO_BOOSTER_TYPES
    .map((type) => byType.get(type))
...
function hudInfoVisibleBoosters(data) {
  return hudInfoOrderBoosters(data?.boosters);
}
...
function hudInfoRenderDataKey(data, settings) {
  return [
    ...
    settings.boosterMergeColor,
    settings.boosterMergeAlpha,
    settings.boosterSpeedColor,
    settings.boosterSpeedAlpha,
    settings.boosterVirusColor,
    settings.boosterVirusAlpha,
    data.score,
    data.averageScore,
    data.fps,
    data.averageFps,
...
export {
  hudInfoBoosterNameColor,
  hudInfoOrderBoosters,
};
