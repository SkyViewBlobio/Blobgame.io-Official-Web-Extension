export function pageVirusMotherCellBootstrap(initialConfig, pageWindow) {
  ...
  const existingState = win.__blobVirusGlowState;
  if (win[INSTALL_KEY] && existingState?.version === config.version) {
    return true;
  }
  ...
  const state = {
    ...
    colorizerVirusCalls: 0,
    colorizerVirusApplied: 0,
    colorizerVirusMissing: 0,
  ...
  function installDebugSnapshot() {
    const debugSnapshot = () => ({
      ...
      commands: [
        'BlobioVirusMotherCellDebug()',
        'BlobioVirusMotherCellPerformanceDebug()',
        'await BlobioRenderPerformanceProfile(3000)',
      ],
      errors: [...state.errors],
    });
    win.__blobVirusGlowDebug = debugSnapshot;
    win.BlobioVirusMotherCellDebug = debugSnapshot;
    try {
      if (win.window && win.window !== win) {
        win.window.__blobVirusGlowDebug = debugSnapshot;
      }
    } catch {}
  }
  ...
  function patchBundle(source) {
    ...
    const fallbackMatch = code.match(FALLBACK_RENDER_RE);
    const drawRegionName = branchMatch?.[3] || fallbackMatch?.[4] || null;
    ...
    if (VIRUS_BRANCH_RE.test(code)) {
      code = code.replace(VIRUS_BRANCH_RE, (match, initDrawState, setColor, drawRegion, branchVirusTexture, offset, fullCode) => {
        ...
        return 'case 4:case 3:'
          ...
          + 'if(h){h.highDetailVirusHits=(h.highDetailVirusHits+1)||1}'
          + 'h=$wnd.__BlobCellColorizer;f=g.K.d;d=g.K.c;b=g.K.b;c=g.K.a;'
          + 'if(h&&h.virus){if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.colorizerVirusCalls=($wnd.__blobVirusGlowState.colorizerVirusCalls+1)||1}if(h.virus(g)){if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.colorizerVirusApplied=($wnd.__blobVirusGlowState.colorizerVirusApplied+1)||1}}else{h=$wnd.__blobVirusGlowSettings;g.K.d=h&&h.texturePreTinted?1:h&&h.r!=null?h.r:1;g.K.c=h&&h.texturePreTinted?1:h&&h.g!=null?h.g:0;g.K.b=h&&h.texturePreTinted?1:h&&h.b!=null?h.b:0;g.K.a=h&&h.alpha!=null?h.alpha:0.85}}else{if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.colorizerVirusMissing=($wnd.__blobVirusGlowState.colorizerVirusMissing+1)||1}h=$wnd.__blobVirusGlowSettings;g.K.d=h&&h.texturePreTinted?1:h&&h.r!=null?h.r:1;g.K.c=h&&h.texturePreTinted?1:h&&h.g!=null?h.g:0;g.K.b=h&&h.texturePreTinted?1:h&&h.b!=null?h.b:0;g.K.a=h&&h.alpha!=null?h.alpha:0.85}'
    ...
    if (FALLBACK_RENDER_RE.test(code)) {
      code = code.replace(FALLBACK_RENDER_RE, (match, fallbackName, initDrawState, setColor, drawRegion, defaultTexture, offset, fullCode) => {
        ...
        return `function ${fallbackName}(a,b){var c,d,e,f,g,h;if(b.c&&(b.c.M==4||b.c.M==3)){`
          + 'c=$wnd.__blobVirusGlowState;'
          + 'if(c){c.fallbackVirusHits=(c.fallbackVirusHits+1)||1}'
          + 'd=b.K.d;e=b.K.c;f=b.K.b;g=b.K.a;h=$wnd.__BlobCellColorizer;'
          + 'if(h&&h.virus){if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.colorizerVirusCalls=($wnd.__blobVirusGlowState.colorizerVirusCalls+1)||1}if(h.virus(b)){if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.colorizerVirusApplied=($wnd.__blobVirusGlowState.colorizerVirusApplied+1)||1}}else{h=$wnd.__blobVirusGlowSettings;b.K.d=h&&h.texturePreTinted?1:h&&h.r!=null?h.r:1;b.K.c=h&&h.texturePreTinted?1:h&&h.g!=null?h.g:0;b.K.b=h&&h.texturePreTinted?1:h&&h.b!=null?h.b:0;b.K.a=h&&h.alpha!=null?h.alpha:0.85}}else{if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.colorizerVirusMissing=($wnd.__blobVirusGlowState.colorizerVirusMissing+1)||1}h=$wnd.__blobVirusGlowSettings;b.K.d=h&&h.texturePreTinted?1:h&&h.r!=null?h.r:1;b.K.c=h&&h.texturePreTinted?1:h&&h.g!=null?h.g:0;b.K.b=h&&h.texturePreTinted?1:h&&h.b!=null?h.b:0;b.K.a=h&&h.alpha!=null?h.alpha:0.85}'
    ...
    const needsCompatibilityFallback = !virusBranchPatched || !fallbackRenderPatched;
    if (needsCompatibilityFallback && drawRegionName) {
      const patchedDrawRegion = patchDrawRegionFunction(
        code,
        drawRegionName,
        settings.rotate ? rotatedDrawName : null,
      );
    ...
    if (RENDER_LOOP_RE.test(code)) {
      const compatibilityState = textureDrawPatched
        ? `$wnd.__blobVirusGlowState.currentCell=null;$wnd.__blobVirusGlowState.virusTexture=${virusTexture};$wnd.__blobVirusGlowState.glowTexture=${glowTexture};`
        : '';
      code = code.replace(RENDER_LOOP_RE, (match) => match.replace(
        ';for(',
        `;if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.frame=($wnd.__blobVirusGlowState.frame+1)||1;${compatibilityState}}for(`,
      ));
      renderLoopPatched = true;
    }

    if (textureDrawPatched && RENDER_CELL_RE.test(code)) {
      code = code.replace(RENDER_CELL_RE, (match) => match
        + 'h=$wnd.__blobVirusGlowState;if(h){h.currentCell=g}');
  ...
  function buildGlowDrawCall(rotatedDrawName, drawRegion, cellName, batchName, textureName, sourceName) {
    ...
    const markDraw = `if($wnd.__blobVirusGlowState){$wnd.__blobVirusGlowState.${drawCounter}=($wnd.__blobVirusGlowState.${drawCounter}+1)||1;if(($wnd.__blobVirusGlowState.${drawCounter}&127)==1){$wnd.__blobVirusGlowState.lastGlowDrawSource='${sourceName}';$wnd.__blobVirusGlowState.${lastCell}={id:${cellName}.n,x:${cellName}.R,y:${cellName}.S,r:${cellName}.M,size:${cellName}.N,type:${cellName}.c?${cellName}.c.M:null,hasName:!!${cellName}.B,u:!!${cellName}.u,rflag:!!${cellName}.r,t:!!${cellName}.t,q:!!${cellName}.q,sourceName:'${sourceName}'}}}`;
    if (!rotatedDrawName || !settings.rotate) {
  ...
  function patchDrawRegionFunction(code, drawRegionName, rotatedDrawName) {
    ...
    return {
      code: code.replace(drawFunction, (match) => match
        ...
        + "g.textureVirusHits=(g.textureVirusHits+1)||1;if((g.textureVirusHits&127)==1){g.lastGlowTextureCell={id:h.n,x:h.R,y:h.S,r:h.M,size:h.N,type:h.c.M,hasName:!!h.B,u:!!h.u,rflag:!!h.r,t:!!h.t,q:!!h.q,sourceName:'texture'}};"
        + (rotatedDrawName
  ...
  function rememberError(error) {
    ...
    win.console?.warn?.('[Blobio Virus/Mothercell glow]', message);
  }
