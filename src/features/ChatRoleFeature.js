...
import { isHideAdminMdEnabled, shouldShowInGameClanText } from '../roles/RoleSettings.js';
import { extractUidFromElement } from '../roles/UidReader.js';
import { createBlobioStorage } from '../storage/BlobioStorage.js';

const EXTENSION_TAG_CLASS = 'blobio-extension-chat-tag';
const MESSAGE_BODY_CLASS = 'blobio-chat-message-body';

function readChatSenderName(message) {
  const firstSpan = Array.from(message?.children || [])
    .find((child) => String(child.tagName).toUpperCase() === 'SPAN');
  return String(firstSpan?.textContent || '').trim().replace(/\s+/g, ' ');
}
...
export class ChatRoleFeature {
  ...
  installRefreshBridge() {
    ...
    this.refreshBridge = () => this.reprocessExistingMessages();
    try {
      win.__blobioChatRoleRefresh = this.refreshBridge;
    } catch {}
  }
  ...
  processMessage(message, force = false) {
    const uid = normalizeUid(message?.getAttribute?.('uid')) || extractUidFromElement(message, true);
    if (!uid) {
      const senderName = readChatSenderName(message);
      const muted = this.mutedPlayersStore?.isNoUidMuted?.(senderName) || false;
      const signature = `no-uid:${senderName}:${muted ? 1 : 0}`;
    ...
    const clanTag = shouldShowInGameClanText(this.storage) ? String(roles.clan?.tag || '').trim() : '';
    const signature = `${uid}:${roles.admin ? 1 : 0}:${roles.vip.active ? 1 : 0}:${clanTag}:${hideAdminMd ? 1 : 0}:${muted ? 1 : 0}`;
    if (!force && message.dataset.blobioRoleSignature === signature) {
      return;
    }
    ...
    if (clanTag) {
      message.insertBefore(this.createTag(` {${clanTag}}`, 'blobio-chat-clan-tag'), messageSpan);
    }
  ...
  destroy() {
    ...
    this.unsubscribeMutedPlayers = null;
    const win = this.document.defaultView || globalThis;
    if (win.__blobioChatRoleRefresh === this.refreshBridge) {
      try {
        delete win.__blobioChatRoleRefresh;
      } catch {
        win.__blobioChatRoleRefresh = undefined;
      }
    }
    this.refreshBridge = null;
