...
import vipBadgeUrl from '../assets/VIP_icon_plus.png';
...
import { getRuntimeBuildChannel, shouldKeepExistingExtension } from './runtime/ExtensionInstancePolicy.js';
...
const EXTENSION_VERSION = '0.2.87';
...
class BlobioExtension {
  constructor(windowRef = globalThis) {
    ...
    this.buildChannel = getRuntimeBuildChannel(windowRef);
    this.features = [];
  ...
  start() {
    ...
    if (hostMode === 'runtime') {
      ...
      this.installCellRingFallback(document, logger);
  ...
  installVirusMotherCellFallback(document, logger) {
    ...
    if (typeof windowRef.__blobVirusGlowDebug !== 'function') {
      windowRef.__blobVirusGlowDebug = debugFallback;
    }
    ...
    try {
      ...
      if (!status.installedByBundle) {
        logger.warn?.('[Blobio] Virus/Mothercell glow runtime did not start. Update or reinstall the Tampermonkey loader, then reload the game tab.');
      }
      return status.installedByBundle;
    } catch (error) {
      status.reason = 'bundle-bootstrap-error';
      status.error = error?.message || String(error);
      logger.warn?.('[Blobio] Virus/Mothercell glow fallback failed.', error);
      return false;
    }
  }
  ...
  installFpsSaverFallback(document, logger) {
    ...
    const storage = createBlobioStorage(document);
    try {
      const installed = Boolean(pageFpsSaverBootstrap(readFpsSaverSettings(storage, document), pageWindow));
      clearRemovedFpsRuntimeHooks(pageWindow);
      return installed;
    } catch (error) {
  ...
  installCellRingFallback(document, logger) {
    const windowRef = this.window;
    const pageWindow = getTampermonkeyPageWindow(windowRef);
    const storage = createBlobioStorage(document);
    try {
      ...
      logger.warn?.('[Blobio] Cell Ring fallback failed.', error);
      return false;
    }
  }
  ...
  installVirusPelletColorsFallback(document, logger) {
    ...
    const existingStatus = pageWindow.__blobioVirusPelletColorLoaderStatus || {};
    const status = {
      ...existingStatus,
      version: EXTENSION_VERSION,
      enabled: settings.enabled,
    ...
    try {
      ...
      status.reason = status.installedByBundle
        ? (settings.enabled ? 'installed-by-bundle-fallback' : 'installed-disabled-by-bundle-fallback')
        : 'bundle-bootstrap-returned-false';
      if (!status.installedByBundle) {
  ...
  installRenderPerformanceFallback(logger) {
    const pageWindow = getTampermonkeyPageWindow(this.window);
    try {
      return Boolean(pageRenderPerformanceBootstrap(pageWindow));
    } catch (error) {
      logger.warn?.('[Blobio] Render performance diagnostics fallback failed.', error);
      return false;
    }
  }
...
function removedFpsRuntimeHookNames() {
  return [
    ['__blob', 'Render', 'Scale', 'Debug'].join(''),
    ['__blob', 'Cell', 'Lod', 'Debug'].join(''),
    ['__Blobio', 'Cell', 'Lod', 'BeginFrame'].join(''),
    ['__Blobio', 'Cell', 'Lod', 'ShouldSimplify'].join(''),
  ];
}

function clearRemovedFpsRuntimeHooks(pageWindow) {
  for (const name of removedFpsRuntimeHookNames()) {
    try {
      delete pageWindow[name];
    } catch {
      pageWindow[name] = undefined;
    }
  }
}

export function startBlobioExtension(windowRef = globalThis) {
  const incomingChannel = getRuntimeBuildChannel(windowRef);
  const existing = windowRef[INSTANCE_KEY];
  if (shouldKeepExistingExtension(existing, EXTENSION_VERSION, incomingChannel)) {
    return existing;
  }
